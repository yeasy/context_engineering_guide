...
标签本身会消耗 Token——每对开闭标签大约 3-6 个 Token。在简单场景中，过度结构化可能得不偿失。例如只有一段参考文本时，用 `--- 参考资料 ---` 这样的简洁分隔符可能比 `<reference_material>...</reference_material>` 更高效。评估标准是：结构化带来的理解准确率提升是否值得额外的 Token 成本。复杂场景（多个数据源、多层指令）值得投入；简单场景保持简洁。

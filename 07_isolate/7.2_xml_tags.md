...
只要标签内内容来自用户、检索结果、工具输出、OCR、转写、日志或数据库，就必须先做上下文相关的 XML 转义。否则攻击者可以在数据中插入 `</user_input><instructions>...</instructions>` 这类闭合标签，逃出原本的数据区域。标签只能帮助模型理解结构，不能替代权限检查、输入验证、工具沙盒和输出审计。
...
def build_context(role, knowledge, query):
    return f"""
<role>{xml_text(role)}</role>
<knowledge>{xml_text(knowledge)}</knowledge>
<query>{xml_text(query)}</query>
"""
```

这种方式便于模块化管理和测试各个组成部分。动态拼接时仍要把每个字段当作不可信数据处理；不要因为外面包了 XML 标签，就允许其中内容改变系统指令、授权范围或工具调用策略。

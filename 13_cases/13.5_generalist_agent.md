...
4.  **权限与副作用 (Permissions & Side Effects)**：全自主智能体会读写文件、调用网络、安装依赖或执行命令；这些动作必须经过权限网关、审计和危险操作审批。
...
-   **轨迹压缩 (Trajectory Compaction)**：
    *   不仅是对结果压缩，对交互历史也进行压缩。当一轮对话结束或任务告一段落，系统会自动将之前的 action、tool result、observation、证据来源和决策摘要压缩为结构化的 `StepSummary`，避免把隐藏推理链当作可交付日志。
...
-   **受治理的文件系统记忆 (Governed Filesystem Memory)**：
    *   智能体运行在一个可持久化的执行环境中。它生成的代码、下载的资料、整理的笔记，可以保存在文件系统中，但必须附带来源、创建者、时间戳、权限标签、保留期限和完整性校验。
    *   Context Window 中只保留“索引”（即文件路径和元数据）。读取文件前先经过权限网关，确认当前任务、用户、租户和工具权限允许访问；敏感文件默认只返回摘要或脱敏片段。
    *   文件系统不是天然可信记忆。网页、用户上传文件、模型生成摘要和工具输出都应标记为不可信或待验证数据，避免被后续任务当成高优先级指令。

-   **状态持久化**：
    *   Agent 的状态不仅存在于对话历史中，更存在于它改变的环境中（安装的包、修改的配置、创建的文件夹）。这种环境副作用（Side Effects）必须可追踪、可回滚、可审计，不能被视为无限且无边界的上下文。
...
-   **Planner-Executor 架构**：
    ...
    *   Planner 不能把“执行权限”直接透传给 Executor；每次工具调用都应经过能力声明、allowlist、路径/网络范围校验、速率限制和审计记录。

-   **权限网关与审批**：
    *   对读文件、写文件、网络访问、包安装、命令执行、发送邮件、提交代码、支付、删除和外部系统写入等操作建立分级权限。
    *   低风险只读操作可自动批准；高影响、不可逆、跨租户、外发数据或修改生产环境的操作需要用户或授权审批人确认。
    *   审批上下文应展示动作、目标、差异、数据分类、风险、回滚方式和预计副作用，而不是只展示一句自然语言请求。
...
graph TB
    ...
    policy["Policy Gateway<br/>(ACL, Approval, Audit)"]
    ...
    executor_mem -.请求访问.-> policy
    policy --授权读写--> fs
    executor_mem --写入请求--> policy

    Planner --读取摘要--> fs
    Executor --工具调用请求--> policy
    policy --批准后执行--> VM[Virtual Machine]
    VM --结果--> fs
...
3.  **分而治之**：通过 Agent 拆分来隔离上下文，可以缓解“迷失在中间”和权限扩散问题，但仍需要共享记忆的权限控制、来源追踪和审批网关。

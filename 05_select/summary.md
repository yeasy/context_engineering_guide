...
| 指标维度 | 指标名称 | 典型目标值 | 说明 |
|----------|----------|------------|------|
| **性能** | 端到端延迟 (P99) | < 2s | 包含检索、排序、生成全流程 |
| | 检索延迟 (P99) | < 200ms | 仅检索环节耗时 |
| | 吞吐量 (QPS) | 视场景定 | 系统每秒处理请求数 |
| **质量** | Recall@K | > 85% | 前 K 个结果中包含正确答案的比例 |
| | MRR (平均倒数排名) | > 0.7 | 正确答案在结果列表中的排名优劣 |
| | 准确率 (Precision) | > 90% | 检索结果的有效性（信噪比） |
| **成本** | 单次查询成本 | $0.001 - $0.01 | 包含 Embedding、存储和 LLM 生成成本 |

...
实际例子：假设使用 Claude Sonnet 4.6（输入价格$0.003/1K token，截至 2026 年 3 月）：

| 上下文长度 | Token数 | Transformer成本 | 相对基准 | SSM成本 | 相同硬件下SSM优势 |
|-----------|--------|-----------------|---------|--------|-----------------|
| 4K | 4000 | $0.012 | 1.0x | $0.012 | 吞吐量提升2-3倍 |
| 8K | 8000 | $0.024 | 2.0x | $0.024 | 吞吐量提升2-3倍 |
| 32K | 32000 | $0.096 | 8.0x | $0.096 | 吞吐量提升6-8倍 |
| 128K | 128000 | $0.384 | 32.0x | $0.384 | 吞吐量提升10-15倍 |
| 256K | 256000 | $0.768 | 64.0x | $0.768 | 吞吐量提升15-20倍 |

注：虽然Token计价相同，**关键差异在吞吐量**。相同硬件下：
- **Transformer**：随着上下文长度增加，单位时间内处理的Token数量下降（二次方复杂度）
- **SSM**：处理吞吐量保持线性增长，同一块GPU在128K上下文下的吞吐量是4K的约30倍

此外，**内存占用** 也显著不同：
- Transformer需存储注意力矩阵（O(n²)内存），64K长度需要4GB+
- SSM仅需固定的隐状态（O(d)内存），同样长度仅需512MB

这使得SSM在 **成本相同** 的情况下能提供更高质量的服务（更快响应、更多并发）。
...
实际性能对比（基于Mamba-2 vs Transformer基准）：

| 指标 | Transformer(32K上下文) | Mamba-2(32K上下文) | 改进 |
|-----|--------|-----------|------|
| 吞吐量(tokens/sec) | 450 | 2800 | 6.2x |
| 内存占用(GB) | 52 | 8.4 | 6.2x |
| 推理延迟(ms) | 180 | 28 | 6.4x |
| 长上下文扩展成本 | 指数增长 | 线性增长 | - |

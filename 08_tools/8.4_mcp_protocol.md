...
MCP Tasks 在 2025-11-25 规范中仍属于实验能力。实际协议还包含 `tasks/get`、`tasks/result`、`tasks/cancel` 等操作；任务结果通过 `taskId` 关联，状态通常在 `working`、`input_required`、`completed`、`failed`、`cancelled` 之间流转。

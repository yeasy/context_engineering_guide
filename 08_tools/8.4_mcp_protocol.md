...
![MCP 协议栈分层图](../_images/ch08-mcp-protocol-stack.svg)
...
#### HTTP Authorization 与安全认证

截至 2026-04-26，MCP 最新规范版本为 `2025-11-25`。MCP Authorization 是 **HTTP-based transport 的可选授权框架**，基于 OAuth 2.1 等标准的一组子集；它不适用于 stdio transport，stdio 场景通常从环境或宿主应用获取凭据。

- **Authorization Endpoint**：用户授权端点，获取授权码
- **Token Exchange**：通过授权码交换访问令牌
- **Resource Indicator**：授权与换取令牌时指定目标 MCP server
- **Bearer Token**：HTTP 请求使用 `Authorization: Bearer <access-token>`
- **Token Validation**：MCP server 校验 token 有效性、audience 和权限范围
- **Scope Control**：细粒度权限控制，限制客户端的访问范围

Authorization 不能替代传输安全。HTTP 场景仍需 HTTPS、PKCE、token 安全存储、受众绑定和最小权限；不能把 token 放在 URL query 中，也不能把发给其他资源服务器的 token 透传给 MCP server。
...
这些机制是 Streamable HTTP transport 的安全要求与建议，不是所有传输的通用机制。
...
用于表示可能持续较长时间的工作单元，例如批量索引、长时爬取或异步分析。相比“一次请求立即返回”的工具调用，任务更强调状态跟踪、轮询、取消和结果延迟获取。按 MCP `2025-11-25` 规范，Task 对象使用 `taskId`，状态值包括 `working`、`input_required`、`completed`、`failed`、`cancelled`；进度通知仍使用 Progress 机制，不放在 Task 的 `progress` 字段里。
...
客户端通过 `tasks/get` 查询状态、通过 `tasks/result` 获取完成后的原始请求结果、通过 `tasks/cancel` 请求取消。HTTP transport 后续请求还应携带协商后的 `MCP-Protocol-Version`，例如 `MCP-Protocol-Version: 2025-11-25`。

...
MCP Tasks 仍属于实验扩展能力，不应写成所有 MCP 客户端和服务端的基线功能。使用前必须通过 capabilities 协商确认双方都支持该扩展。扩展的典型生命周期是：发起长任务后立即返回 `taskId`；客户端用 `tasks/get` 查询状态和进度；需要人工或模型补充输入时进入 `input_required`，再用 `tasks/update` 提交补充输入；完成后 `tasks/get` 在终态响应里带回 `result` 或 `error`；必要时可用 `tasks/cancel` 取消。状态名和结果字段要以所采用的扩展版本为准。
...
配置文件只声明如何启动或连接 MCP server；`resources`、`tools`、`prompts` 应由 server 通过 MCP 协议发现和暴露，不应静态写入客户端配置。

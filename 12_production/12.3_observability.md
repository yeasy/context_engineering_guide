...
- **检索 Spans**：记录向量检索、关键词检索等操作，包括查询哈希、查询长度、查询类别、检索文档 ID、相关性得分
- **工具调用 Spans**：记录每次工具调用的工具名、参数 schema 版本、执行时间、状态码、错误码；默认不记录原始参数和返回体
- **提示词与完成度 Spans**：默认只记录 prompt/response/context 的哈希、长度、类别、trace id、模型名与模型版本、错误码；不记录完整提示词、完整响应或完整上下文
...
生产环境的观测数据要遵循数据最小化：加密不是记录原文的理由。只有在独立的调试通道中，且同时满足采样、自动脱敏、短 TTL、RBAC、工单审批和访问审计时，才允许临时保存原文片段。

#### 工件日志与可重放性

为了支持完整的可审计性和调试，重要中间结果应该以 **工件（Artifacts）** 的形式记录其可验证元数据，而不是默认持久化原文：

- **结构化格式**：使用 JSON 或 Protocol Buffers 等机器可读的格式，而不仅仅是文本日志
- **校验和与版本**：记录工件的内容哈希值和版本号，确保可重放时的一致性
- **安全快照**：在关键决策点保存状态、哈希、长度、类别、trace id、模型/版本、错误码、检索文档 ID/score 等；默认不保存提示词、响应、上下文或工具输入输出原文
...
保存关键请求的安全快照。默认只保存不可直接还原用户内容的元数据：
...
if should_snapshot(request):
    save_snapshot({
        ...
        "response_hash": sha256(response.encode()).hexdigest(),
        "response_length": len(response),
        "model": model_name,
        "model_version": model_version,
        "error_code": error.code if error else None,
        "retrieved_docs": [
            {"doc_id": doc.id, "score": round(doc.score, 4)}
            for doc in docs
        ],
    })
```
...
如果必须保存原文用于事故复现，应写入独立的受控调试存储，并满足以下条件：低比例采样、DLP/PII/PHI 脱敏、明确 TTL、最小权限 RBAC、审批单号、访问审计和到期自动删除。

...
# - OpenAI 官方 API pricing：GPT-5.4 input $2.50/1M、cached input $0.25/1M、output $15/1M；
#   GPT-5.4 mini input $0.75/1M、cached input $0.075/1M、output $4.50/1M。
# - Anthropic 官方 prompt caching docs：Claude Sonnet 4 input $3/1M、5m cache write $3.75/1M、
#   cache read $0.30/1M、output $15/1M；Claude Opus 4.1 input $15/1M、5m cache write
#   $18.75/1M、cache read $1.50/1M、output $75/1M。
# - 下表统一换算为美元/1K tokens；未列 Batch、regional processing、工具调用、长上下文额外费用。
PRICE_TABLE = [
    ModelPricing(
        ...
        cache_write_price_per_1k_tokens=0.0025,         # OpenAI 未单列写入价；未命中按普通输入价
    ...
    ModelPricing(
        ...
        cache_write_price_per_1k_tokens=0.00075,
    ...
    ModelPricing(
        ...
        cache_write_price_per_1k_tokens=0.015 * 1.25,
        cache_read_price_per_1k_tokens=0.015 * 0.1,
    ),
    ModelPricing(
        provider=ModelProvider.ANTHROPIC,
...
class TokenCostCalculator:
    ...
    def calculate_single_request_cost(
        ...
        if not use_cache:
            ...
            # 缓存未命中但要写入缓存：按 cache write 价格计费。
            # Anthropic 的 cache write 是一个替代输入价，不是在普通输入价之上再加一次。
            # OpenAI 未单列 cache write 价格时，上表把 write 价设为普通 input 价。
            cache_miss_cost = 0.0
            ...
            return {
                ...
                'cache_cost': cache_hit_cost + cache_write_cost,

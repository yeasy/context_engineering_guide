...
![端到端安全数据流图](../_images/ch12-security-data-flow.svg)
...
class SensitivityScorer:
    ...
    def __init__(self):
        ...
        self.sensitive_patterns = {
            'email': r'[\w\.-]+@[\w\.-]+\.\w+',
            'phone': r'\+?1?\d{9,15}',
            # 仅用于发现疑似 PAN 候选，不代表 PCI DSS 合规。
            'payment_card_candidate': r'\b\d{13,19}\b|\b\d{4}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{1,7}\b',
    ...
    def redact_output(self, output: str, aggressive: bool = False) -> str:
        ...
        redaction_rules = {
            'email': '[邮箱地址已隐藏]',
            'phone': '[电话号码已隐藏]',
            'payment_card_candidate': '[支付卡号候选已隐藏]',
            'ssn': '[社保号已隐藏]',
            'api_key': '[API密钥已隐藏]',
            'password': '[密码已隐藏]',
        }
...
注意：支付卡号候选检测只是 DLP/脱敏的一部分，不等于 PCI DSS 合规。PCI 范围还取决于系统是否存储、处理或传输 CHD/SAD，SAD 授权后不得留存，PAN 需要按业务目的做截断、令牌化、强访问控制、分段和正式合规评估。
...
class SanitizedLogger:
    ...
    def log_retrieval_event(
        ...
        log_entry = {
            ...
            'retrieved_docs': [
                {'doc_id': doc_id, 'score': round(float(score), 4)}
                for doc_id, score in zip(retrieved_doc_ids, scores)
            ],
            # 故意不记录：具体的向量值、文档正文、查询原文
        }
...
class PIIDetector:
    ...
    def __init__(self):
        self.patterns = {
            'email': r'[\w\.-]+@[\w\.-]+\.\w+',
            'phone': r'(?:\+?86[-\s]?)?1[3-9]\d[-\s]?\d{4}[-\s]?\d{4}|(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}',
            'id_number': r'\d{6,18}',  # 身份证、学号等
            'name_pattern': r'[A-Z][a-z]+ [A-Z][a-z]+',  # 英文全名
            'medical_record': r'(病历号|住院号|门诊号|诊断|医嘱|MRI|CT|检验结果)[\s:：]*[^\n，。；;]+',
        }
...
class ContextPIIPipeline:
    ...
    def _build_patterns(self) -> Dict[PIICategory, List[str]]:
        ...
        patterns = {
            ...
            PIICategory.CREDIT_CARD: [
                # 疑似 PAN 候选；PCI DSS 合规还需要范围界定、SAD 禁存、PAN 截断/令牌化、访问控制和评估。
                r'\b\d{13,19}\b|\b\d{4}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{1,7}\b',
            ],
            PIICategory.ADDRESS: [
            ...
            PIICategory.MEDICAL_RECORD: [
                r'(病历号|住院号|门诊号)[\s]*[:：]?\s*[A-Za-z0-9\-]+',
                r'(诊断|医嘱|过敏史|检验结果|影像所见|MRI|CT)[\s]*[:：][\s]*[^\n，。；;]+',
                r'患者[\u4e00-\u9fff]{1,4}.*?(诊断|病历|住院|门诊)',
            ]
        }
    ...
    def filter_retrieval_documents(
        ...
        self.pii_log.append({
            ...
            'query_pii_categories': [cat.value for cat, _ in query_pii],
            'user_role': user_role,
...
# 检测查询中的 PII
user_query = "患者张三的邮箱是 zhangsan@example.com，电话是+86-138-1234-5678，病历号 MR-12345，诊断: 糖尿病"
...
class AuditEvent:
    ...
    details: Dict  # 额外细节；只允许 hash、长度、类别、错误码、doc_id/score 等安全元数据
...
审计日志应服务于可追溯性，而不是成为敏感数据副本。默认禁止在 `details` 中写入原始 query、prompt、response、context、病历正文、身份证号、完整 PAN 或工具返回体；如需事故级原文调试，应使用单独的短 TTL 调试存储并经过审批。

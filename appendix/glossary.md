...
**Context Rot（上下文腐烂）**
随着上下文窗口中 Token 数量增加，模型对关键信息的提取与回忆稳定性可能下降的现象。本书用此概念来强调上下文应被当作稀缺资源管理。

**Context Stuffing（上下文填塞）**
将大量未经筛选的信息填入上下文窗口的做法，通常会降低模型输出质量、增加成本和延迟。是常见的上下文工程反模式。
...
一种开放的标准协议，用于规范 AI 模型或客户端与外部工具、资源和提示模板的交互；该协议最早由 Anthropic 推动并向社区开放。它本身是开放规范，不应被理解为 Anthropic 私有协议。

**Memory Architecture（记忆架构）**
组织和管理 AI 系统记忆的多层次结构设计。

**MemOS（记忆操作系统）**
一种把记忆管理操作封装为标准化服务、对应用开发者透明的记忆操作系统。其论文采用接口层、操作层、基础设施层的三层架构（本书 4.2.4 节为便于讲解，进一步拆分为五层工程视角）。

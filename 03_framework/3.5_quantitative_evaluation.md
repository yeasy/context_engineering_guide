...
class RelevanceScorer:
    ...
    def ndcg(
        ...
        k: int = 10,
        ideal_relevance_scores: List[float] = None
    ) -> float:
        """
        ...
        其中IDCG是完整标注集合在完美排序下的DCG。
        如果只传入返回结果的 relevance_scores，NDCG 只是在“已返回集合”内归一化；
        评估检索系统时应传入该 query 的完整 ground truth relevance 列表。
        ...
        # 计算理想的DCG：用完整标注集合排序，而不是把返回结果当作全集。
        if ideal_relevance_scores is None:
            ideal_relevance_scores = relevance_scores

        sorted_scores = sorted(ideal_relevance_scores, reverse=True)
        ideal_dcg = 0.0
...
if __name__ == "__main__":
    ...
    # ground truth 标注集中共有6个相关项，当前只返回了其中4个。
    relevant_count = 6
    ...
    print(f"Recall@10: {recall:.2f}")  # 应该是0.67（找到4/6）
...
class GradedRelevanceScorer:
    ...
    def ndcg_with_grades(
        ...
        # 理想的DCG：同一批已标注项目按真实相关性降序排列。
        # 不能把所有项目都替换成最高等级，否则会高估IDCG、低估NDCG。
...
class RetrievalQualityEvaluator:
    ...
    def evaluate_retrieval(
        ...
        ground_truth_relevant_count: int,
        top_k_values: List[int] = None
    ) -> dict:
        """
        对单个查询的检索结果进行多维度评估。

        ground_truth_relevant_count 必须来自完整标注集或 qrels。
        不能用返回结果中的相关数作 Recall@k 分母，否则漏召回会被隐藏。
        """
        ...
        retrieved_relevant_count = sum(is_relevant)
        relevant_count = ground_truth_relevant_count
    ...
    def evaluate_retrieval_batch(
        ...
        ground_truth_relevant_counts: List[int],
        top_k_values: List[int] = None
    ) -> dict:
        """
        ...
        for query, results, relevant_count in zip(
            queries,
            retrieved_results_batch,
            ground_truth_relevant_counts
        ):
            indiv = self.evaluate_retrieval(
                query,
                results,
                relevant_count,
                top_k_values
...
class ABTestFramework:
    ...
    def paired_ttest(
        ...
        同一批 query 分别跑 control/treatment 时，两个分数序列一一对应，
        应使用 paired t-test，而不是 independent t-test。
        SciPy 口径核验日期：2026-04-26，stats.ttest_rel 用于 two related samples。
        """
    ...
    def cohen_d(
        ...
        """
        计算配对样本效果量 Cohen's dz

        衡量两组差异的实际大小（不只是统计显著性）。
        """

        differences = np.array(treatment_group) - np.array(control_group)

...
某研究团队的实验表明，用户提示词的有效性可以通过视觉提示增强：

```text
试验 1（纯文本）：
用户：请读取以下文本并总结
成功率：0%（模型拒绝，因为提示词包含注入）
...
| 模型 | 隐藏文字识别 | 背景细节提取 | 个人识别 | 安全设置 |
|-----|----------|----------|--------|--------|
| GPT-4V | 强（>95%） | 强 | 中等 | 有 |
| Claude Vision | 强（>90%） | 强 | 低 | 强 |
| Gemini 3 Pro | 很强 | 很强 | 强 | 中等 |
| 开源 CLIP | 中等 | 中等 | 低 | 无 |

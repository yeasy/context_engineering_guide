...
某个 500K 文档的知识库（使用 Claude 的当前高性能档位，按 token 而不是按调用计价；以下以输入 token 价作为保守下限）：
- 初始构建：约 10B input tokens × $3/1M tokens = $30K（若包含大量输出，还要叠加更高的 output token 费用）
- 月度更新：约 2B input tokens × $3/1M tokens = $6K/月
- 查询时（100K月查询）：可能需要额外 1B input tokens × $3/1M tokens = $3K/月

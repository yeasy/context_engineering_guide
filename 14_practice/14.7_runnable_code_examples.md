...
class Memory:
    ...
    def __init__(
        ...
        """
        tenant_id/user_id: 记忆隔离边界，所有读写都必须匹配。
...
class SimpleMemorySystem:
    """
    简单的记忆系统。
    用于存储和检索对话中的关键信息。生产系统必须把 tenant_id/user_id
    作为索引、检索、导出和删除的硬边界，不能共用全局记忆池。
    """
    ...
    def store_memory(
        ...
        存储一条新记忆。
        """
        memory = Memory(tenant_id, user_id, content, memory_type, importance)
        self.memories.append(memory)

        # 如果超过容量，删除最不重要的记忆
        if len(self._scope_memories(tenant_id, user_id)) > self.max_memories:
            self._cleanup(tenant_id, user_id)

        return memory

    def recall(self, tenant_id: str, user_id: str, query: str, top_k: int = 3) -> List[Memory]:
        """
        回忆相关的记忆。
        简化版本：基于关键词匹配。
        实际应用应使用向量相似度。
        """
        # 清理过期记忆
        self._cleanup_expired(tenant_id, user_id)

        # 简单的关键词匹配
        query_words = set(tokenize_memory_text(query))
        scored_memories = []

        for memory in self._scope_memories(tenant_id, user_id):
            memory_words = set(tokenize_memory_text(memory.content))
    ...
    def _scope_memories(self, tenant_id: str, user_id: str) -> List[Memory]:
        """只返回当前租户和用户的记忆，防止跨用户召回。"""
        return [
            mem for mem in self.memories
            if mem.tenant_id == tenant_id and mem.user_id == user_id
        ]

    def _cleanup(self, tenant_id: str, user_id: str) -> None:
        """
        清理：删除最不重要的记忆以腾出空间。
        重要性评分 = 重要度 + 访问频率 - 年龄衰减
        """
        current_time = datetime.now()
        scoped_memories = self._scope_memories(tenant_id, user_id)
        ...
        # 按优先级排序，保留分数最高的
        scoped_memories.sort(key=lambda x: getattr(x, 'priority_score', 0), reverse=True)
        keep_ids = {mem.id for mem in scoped_memories[:self.max_memories]}
        self.memories = [
            mem for mem in self.memories
            if mem.tenant_id != tenant_id or mem.user_id != user_id or mem.id in keep_ids
        ]

    def _cleanup_expired(self, tenant_id: str, user_id: str) -> None:
        """删除过期的记忆"""
        current_time = datetime.now()
    ...
    def export_memories(
        ...
        """
        导出记忆为JSON格式。
        默认只导出元数据；明文导出必须是本人或租户管理员，并提供审批号。
        """
        is_owner = requester_user_id == user_id
        is_tenant_admin = requester_role == "tenant_admin"
        if not (is_owner or is_tenant_admin):
            raise PermissionError("Only the owner or tenant admin can export memories")

        if include_content and not approval_id:
            raise PermissionError("Plaintext memory export requires an approval_id")

        exported = []
        for mem in self._scope_memories(tenant_id, user_id):
            item = mem.to_dict()
            if not include_content:
                item['content_hash'] = hashlib.sha256(mem.content.encode()).hexdigest()
                item['content_length'] = len(mem.content)
                item.pop('content', None)
            item['export_approval_id'] = approval_id
            exported.append(item)

        return json.dumps(exported, indent=2, ensure_ascii=False, default=str)

    def import_memories(self, tenant_id: str, user_id: str, json_str: str) -> None:
        """从JSON导入记忆"""
        data = json.loads(json_str)

        for item in data:
            if item.get('tenant_id') != tenant_id or item.get('user_id') != user_id:
                raise PermissionError("Cannot import memories across tenant/user boundary")
            if 'content' not in item:
                raise ValueError("Cannot import metadata-only memory export")
    ...
    def get_context_for_conversation(
        ...
        为对话生成包含记忆的上下文。
        """
        recalled = self.recall(tenant_id, user_id, user_input, top_k=5)
...
if __name__ == "__main__":
    ...
    # 存储一些记忆
    memory.store_memory(tenant_id, user_id, "用户名是张三", memory_type="fact", importance=0.9)
    memory.store_memory(tenant_id, user_id, "用户来自北京", memory_type="fact", importance=0.7)
    memory.store_memory(tenant_id, user_id, "用户喜欢Python编程", memory_type="preference", importance=0.8)
    memory.store_memory(tenant_id, user_id, "用户是数据科学家", memory_type="fact", importance=0.8)

    # 检索相关记忆
    recalled = memory.recall(tenant_id, user_id, "告诉我关于用户的信息")
    print("回忆起的记忆:")
    for mem in recalled:
        print(f"  - {mem.content}")

    # 为对话生成上下文
    context = memory.get_context_for_conversation(tenant_id, user_id, "你好，有什么问题吗？")
    print("\n对话上下文:")
    print(context)
```

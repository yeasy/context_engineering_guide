...
本章将通过一个端到端教学案例，带领大家构建企业知识库问答系统的核心路径。我们将综合运用前面各章节学到的知识，包括 RAG 架构、上下文压缩、记忆管理、工具调用等，说明可靠 AI 应用的主要工程边界。

本章代码分为两层：正文解释生产系统的设计边界，配套 [EnterpriseKnow 可运行最小实验](../examples/enterprise_know/README.md) 提供可本地执行的检索、权限过滤、来源引用和评估集回归闭环。生产化还需要补齐租户级权限、数据脱敏、运维告警、依赖锁定和发布流程。
